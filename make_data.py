...
#     return blank
def makebackground():
    background_path='./yymnist/back'
    imglist=os.listdir(background_path)
    randpic = random.choice(imglist)
    imgpath=background_path+'/'+randpic
    img=cv2.imread(imgpath)
    img = cv2.GaussianBlur(img, (3, 3), 0)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    binary = cv2.adaptiveThreshold(gray, 255,
                                   cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 15, -10)
    binary=cv2.resize(binary,(416, 416))
...
with open(flags.labels_txt, "w") as wf:
    ...
    while image_num < flags.images_num:
        ...
        #     idx = random.randint(0, 1998)
        #     data[0] = make_image(data, image_paths[idx], ratio)

        #  medium object
        ratios = [0.9,1.1, 1.3, 1.4]
        N = random.randint(4, 10)
        if N !=0: bboxes_num += 1
        for _ in range(N):
            ratio = random.choice(ratios)
            idx = random.randint(0, 1090)
            data[0] = make_image(data, image_paths[idx], ratio)
